        ...
        try:
            if value == "":
                ...
                try:
                    ...
                    if withdrawResponse != 0 and withdrawResponse != "Invalid message":
    ...
    def deposit(self, value):
        try:
            if value == "":
                ...
                try:
                    value = Decimal(value)
                    depositResponce = atm.deposit(authCode, Decimal(value))
                    if depositResponce != 0 and depositResponce != "Invalid message":
                        popupSuccess("You have successfully deposited: " + depositResponce)
                        self.manager.current = 'choose'
                        self.manager.get_screen('choose').updateBalance()
                    else:
                        popupError("Error with request")
